...
**GDN-2 + Mamba-3 (2026-07-08 웹 확인 — 컷오프 이후 신규, 인용 필수):** Gated DeltaNet-2 (arXiv 2605.22791, NVIDIA, 2026-05, NVlabs/GatedDeltaNet-2) = erase/write를 **채널별 게이트로 분리**해 GDN·KDA를 일반화(GDN·KDA의 공통 한계 = erase-write 스칼라 결박 해소). 1.3B/100B FineWeb-Edu, baselines Mamba-2/GDN/KDA/**Mamba-3**, eval = commonsense + retrieval(recall-intensive) + RULER S-NIAH(multi-key 강점). 우리와의 관계: ① T8 가족일반성 2×2 매트릭스의 새 셀(채널별 분리 erase+write) — retrofit/nesting 일반성 검증 확장 후보. ② 그들의 eval 스택 = 이 분야 표준(commonsense 8 + recall-intensive 6 + RULER) — 우리 acc 판정도 이 스택으로 맞춰야 리뷰 방어됨(2026-07-08 커버리지 확장 착수). ③ state N 자체의 runtime 신축/티어링은 여전히 안 다룸 — novelty 무손상.
