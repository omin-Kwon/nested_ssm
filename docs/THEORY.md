...
**정렬 대상의 정체**: 방향 $r_n$의 기대 readout 기여 = $r_n^\top M r_n$, $M=\mathbb{E}[C B^\top \cdot w]$ (w=value·decay·staleness 가중). prefix-loss 최적 R = **M(대칭화)의 고유기저, 고유값(=트래픽) 내림차순** — 레이어의 "통신 버스"를 트래픽 순으로 배열. nested-dropout→PCA(명제 1)의 연산자 형태.
