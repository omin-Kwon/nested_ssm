...
## 해석 — 정렬되는 것은 무엇이고, 왜 성립하는가 (2026-07-14 문답 정리)

**정렬 대상의 정체**: 방향 $r_n$의 기대 readout 기여 = $r_n^\top M r_n$, $M=\mathbb{E}[C B^\top \cdot w]$ (w=value·decay·staleness·**loss-민감도** 가중). prefix-loss 최적 R = **M(대칭화)의 고유기저, 고유값(=트래픽) 내림차순** — 레이어의 "통신 버스"를 트래픽 순으로 배열. nested-dropout→PCA(명제 1)의 연산자 형태.

**⚠ 측정에 의한 정밀화 (E-T1/E-T1b, 2026-07-14)**: w=1(크기)로 명시 추정한 M — lag-0과 시차-가중(ā=0.9/0.99) 모두 — 의 고유상위 32공간은 학습 R의 hot 부분공간과 **무상관**(주각 cos 0.437~0.438 ≈ 무작위 0.5 이하; gap@32≈0). 즉 **w가 보정이 아니라 지배 인자** — 열노름(ktop 붕괴)·질의에너지(ρ 무신호)까지 4중 수렴으로: "측정 가능한 training-free 통계는 학습 기저를 예측 못 한다; 중요도는 loss가 정의하며 end-to-end 학습은 생략 불가." (GHOST류 calibration 접근의 실패 −26.5의 이론적 뿌리이자, 우리 retrofit 학습의 필요성 증명.)

**⚠ 일반화 근거의 재배선 (E-T2, 2026-07-14)**: Davis-Kahan 갭 논증(명시 스펙트럼의 n=32 경계 갭이 이식성 보장)은 gap@32≈0 실측으로 **기각**. 실제 일반화 근거 = ① 가설류 작음(직교군 0.04%, 좌표 선택일 뿐) + ② **이식의 직접 실측**: 학습 R(longcot2) 하나의 절단세 곡선(vppl-tax vs 폭)이 wiki/수학/코드에서 사실상 일치 — k16: 1.26/1.31/1.27×, k32: 1.15/1.21/1.18× (도메인 산포 ≤0.06). 대조 calibration 순열(ghost)은 자기 절단폭(32) ppl만 생존(1.27~1.61×; 그마저 acc는 −26.5로 붕괴)하고 k16 이하 절벽(17~36×) — **nested 탄력성 자체가 학습 산물**. 추가 관측 = **메뉴 격자(flag)**: off-menu 폭(8, 96)은 3도메인 공통 유료(k96 1.2~1.4× > k64) → 학습물은 '고유값 전순서'가 아니라 학습 메뉴 {16,32,64,128} 지점의 중첩 부분공간 사슬. E-T1(어떤 M의 고유기저와도 무상관)과 정합 — R이 만족해야 하는 건 메뉴 경계 제약뿐이며 그 안의 회전 자유도는 미결정. (원장 E-T2 절, results/domain_elasticity.json.)

**성립의 세 힘**: ① B,C는 같은 hidden 스트림의 고정 선형 그림자 → LM 표현의 스펙트럼 집중을 M이 상속. ② 사전학습이 이미 검색 트래픽을 소수 기능 채널로 몰아놓음 (GHOST "phantom states"가 독립 증거). ③ backbone 동결+언어 정상성 → M은 고정 객체, 훈련 분포의 고유기저가 테스트의 모든 새 토큰에 유효 ("**생성기를 정렬하지, 내용을 정렬하지 않는다**" — 상태가 6k회 갱신된 뒤에도 lossless인 이유; 단 분포 조건부 — domain-mix 필수 실측이 경계).

**핵심 문장**: R은 집중을 만들지 않는다 — 집중은 사전학습이 만들었고, R은 그것을 **좌표축에 정렬**시켜 연속 슬라이스(하드웨어가 다룰 수 있는 형태)로 바꿀 뿐이다. (회전-retrofit ≈ from-scratch nested 실측 = 이 문장의 증명.)

**꼬리의 성질**: M의 스펙트럼은 급감하나 두터운 꼬리 — 평균 트래픽은 작고, per-token 지지집합은 밀집·예측불가 (ktop 붕괴 81→21, 삭제 −26.5). 이 프로필의 유일한 적정 관리 = 지우지도 고르지도 않는 **전량 유지 + 비대칭(lazy·저정밀) 관리** — hot/cold 설계의 존재 이유. 비유의 정확한 형태: short/long-term memory가 아니라 **learned write-back cache hierarchy** (분할 기준이 시간이 아니라 트래픽; flush=write-back).
