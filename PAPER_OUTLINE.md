...
- **Sparse linear attention (SSE arXiv:2507.16577, MoM arXiv:2502.13685)**: 같은 동기(state=recall), 다른 축 — 그들은 토큰별 라우팅으로 **무엇을 저장할지 선택**(sparse-and-fresh: 비가역 손실, state 전부 HBM 상주, gather/scatter), 우리는 **언제 읽을지만 미룸**(dense-but-stale: 오차 age-국소·정확보상, capacity 해방, dense streaming, controller-less). 직교라 결합 가능(그들의 확장 state도 K×V 직합 → nesting+티어링 적용 대상).
...
- [x] **Motivation 실측 그림(§2용, `scale/state_bound_motivation.png`, 80/160GB 풀예산 실측 완료):** fla decode 커널, idle B200 — **80GB: 50.4k→1.6k tok/s (state 25→805MB, 31.6× at 32× = slope −1 정확)**, 커널 BW **4.96-4.99TB/s 전 구간 고정**(memory-bound 직접 증거), **160GB(용량 2×, batch 2×) → tok/s 동일(+1~5%)** — "용량 공급만으론 throughput 불변, 천장은 BW÷state" 한 그림 완결. GDN·GLA×2예산 4곡선이 1/state 기준선에 완전 중첩. (부록: 소예산 8/16/24GB 곡선 + batch=1 capacity wall + dk축 커널 열화, b200_ref/)
- [x] 실GDN retrofit: 회전-only 50초 FT = from-scratch 동등 (k8 0.863 vs 0.871)

- [x] **배치 다이얼 곡선(pb-sweep, `scale/pb_sweep.py` 로그):** 정확도-vs-hot비율은 비례 손실이 아니라 **무릎(knee)** — GDN: hot 75/50/25/12% → +0.2/+0.5/+2.0/+4.6%, hot 6% → +65~72%, hot 0% → 7~9× / M2: 12%까지 평평(+1.3%), 6% → +95~110%, 0% → 32~48×. c(8/16)는 무릎 위에선 무영향. **판정: hot을 ~12%까지 줄여도 ≤5% — §8 설계점(128/1024=12.5%)이 실측 안전 경계와 일치.** 무릎 위치 ≈ 학습 폭 메뉴의 최소폭(8) 직하 — "메뉴에 4를 넣으면 무릎이 내려간다" 가설(미검증, future). "cold 버림(k16: ppl +3~11%, needle -57%) vs cold 낡게 씀(~0%)" 대비 = cold의 가치는 staleness에 살아남고 제거에 죽는다.
