...
### T12 완료 — 다음 후보 (우선순위 순)
- **논문 §5/§4 수치 배선**: T12 헤드라인(retrofit acc 공짜 71.4→71.3, v4-c16 −3.7pt, needle v4-c64 0.67→0.96, hot-alone 0.08→0.46) → PAPER_OUTLINE 해당 구멍에 기록.
- **c=8 설계점 재측정**: lm-eval·needle을 c=8(설계점)로 — c=16보다 완만할 것(현재 c16만 측정). 가벼운 후속.
- **v4-aware 예산 확장**: 2k 스텝은 v4 ppl 비용 +2.3~5% 잔존 → 더 많은 스텝/큰 hot으로 0 수렴 검증(예산부족 서사 완결).
- **Arora 컨택**: `CONTACT_ARORA.md` 최종본(발송 전 Stuart Sul 선연락→지도교수 승인). 이제 9B 다운스트림 수치까지 확보돼 서사 완결도 높음.
- **커널 실측 §8 / H100 절대 앵커**: arms 절대값 앵커(엔진 자기모순 각주 해소)는 여전히 미해결.
