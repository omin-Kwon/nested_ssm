...
- **밤샘 최종 판정(limit=1000)**: base 69.57 / P4 69.75 / P4-long 69.40 — 통계적 동률(limit500의 하락은 샘플 노이즈). **P4-long(`nemo9b_rot_p4long.pt`) = 최종 배포 ckpt**: v4-c4 ppl 0% + needle 1.00/1.00 + 폭 최강 + 다운스트림 중립. **최종 배포점: v4-c4 + bf16-cold = 실측 1.92×, 다운스트림 fresh 대비 ~−1pt급.** 교훈: 후보 간 다운스트림 비교는 limit≥1000.
