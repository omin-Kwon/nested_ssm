...
- **[밤 마감 시점 진행 중]** ① fineweb 24M tok 프렙(`prep_fineweb_nemo.py`→`mixed_train_nemo.npy`, prep_fineweb.log) ② P2 c4-fp32 컨트롤(night_lmeval_p2fp32.log). **완료 시**: mixed 데이터로 tdecay 1200스텝 재학습(`--data mixed_train_nemo.npy --tune_decay --cs_menu 4 8 16 32 64 --resume nemo9b_rot_v4aware.pt`) → nemo9b_eval + lm-eval c4-bf16 — 목표: in-domain 이득 유지 + 다운스트림 70.5+ 회복.
