...
**지금 실행 중 (T8, 완료 시 알림): KDA/Mamba2 가족 nested 쌍 4런** — `scale/kda_lm.py`(Kimi 가족: 채널decay+delta, fla KimiDeltaAttention, in-kernel l2norm이라 GDN 마스킹 레시피 그대로) + `scale/m2_lm.py`(Nemotron 가족: chunk_simple_gla=Mamba2 SSD, 스칼라decay+additive, 명시적 qk-norm 필요) × {nested, fixed64}, 30k steps wt103, GDN/GLA 쌍과 동일 shape(d512 L6 H8 dk64 dv128). 로그 `kda_lm_*.log`, `m2_lm_*.log`. **완료 시: GLA식 eval(ppl vs width + arms)로 판정 — 예측: 탄력성 tax ~4-5%, v4는 KDA=관리 필요(delta)/M2=0%(additive), c1 붕괴 재현. 이러면 4가족(GDN/GLA/KDA/M2) 일반성 완성.**
