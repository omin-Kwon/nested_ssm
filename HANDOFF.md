...
- **정밀도 공정성 [진행 중, knob_bf16state.log]**: fp32 state가 fla/mamba 표준이지만 우리 판정에 bf16-state ppl 무사(7.51) 단서 → per-token bf16 라운딩의 **needle**(나이 400+) 프로브. 깨지면 fp32-fresh baseline 정당(현 수치 유지 + 비대칭 정밀도 면허 증거), 버티면 baseline을 bf16-fresh로 재앵커하고 **fp8-cold**(라운딩÷c 논거)로 ~2× 복원. 문헌: Quamba/MambaQuant(state 양자화)와 합성 관계 — 우리 기여 = "staleness 구조가 만드는 티어별 비대칭 정밀도 예산".
