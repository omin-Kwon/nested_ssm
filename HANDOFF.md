...
- **★★P4-long 판정: v4-c4 ppl = fresh와 0% 동률 + needle 1.00/1.00 + 폭 최강** (`nemo9b_rot_p4long.pt`) — "tiered 실행 공짜"가 9B ppl에서 문자 그대로 성립. 다운스트림 점추정은 70.07(기준 70.47, 노이즈 내 단조 하락 — 잔여 도메인 압력 시사).
- **밤샘 최종 판정(limit=1000)**: base 69.57 / P4 69.75 / P4-long 69.40 — 통계적 동률(limit500의 하락은 샘플 노이즈). **P4-long(`nemo9b_rot_p4long.pt`) = 최종 배포 ckpt**: v4-c4 ppl 0% + needle 1.00/1.00 + 폭 최강 + 다운스트림 중립. 교훈: 후보 간 다운스트림 비교는 limit≥1000.
- **★★★ 다운스트림 gap 봉합 (warmup window)**: 잔존 −1pt = 짧은-시퀀스 아티팩트 → **첫 W토큰 fresh 후 v4 전환**(--warm 64; 배포 의미론 = §8 one-time cold-ship). **실측 L1k: fresh 70.42 vs p4long+warm64 70.35 = 동률(−0.07).** 긴-시퀀스 증거 독립 성립(ppl 0%/needle 1.00/lambada ≈).
- **★★★ warmup은 c축 전체를 무손실화**: c8+warm64 70.35(=fresh) / **c16+warm64 70.25(−0.17 노이즈)** → **최종 배포점 = v4-c16 + bf16-cold + warm64 = 실측 2.42×, 다운스트림 fresh 동률, needle 1.00, ppl +0.3%** (각주: lambada 65.5 vs 66.7 — 최장 태스크 미세 흔적). 논문 §5 헤드라인.
- **pb16 정찰(재학습 없이)**: ppl +2.5~4.8%(무릎 잔존 — pb32 학습 탓), needle 1.00/0.96 생존 → **pb 메뉴 {16,32} 재학습이 ~2.7× 후보** (대기열).
- **★★★ 비대칭 정밀도 면허 실측 완료 (정밀도 공정성 판정)**: ① bf16-fresh 깨끗(ppl 불변·needle 1.00) → 정직 baseline = bf16-fresh, fp32 대비 1.92×/2.42×는 "표준 구현" 층. ② fp8은 스케일링 필수(원시 캐스트 NaN — e4m3 448 < state absmax). ③ **스케일드 fp8: per-token +5.1%·needle 0.92 ↔ cold snapshot 0%·needle 1.00/1.00 — 같은 정밀도가 baseline은 깨고 cold는 공짜(주입빈도 ÷c).** ④ 재앵커 헤드라인: **v4-c16+fp8cold+warm64 ≈ 2.1× vs bf16-fresh**(트래픽 0.47S vs 1.0S). **fp8-cold 다운스트림 통과(70.07≈fresh)** — 전 축 공짜. **최종 검증 배포 스택: p4long + v4-c16 + scaled-fp8-cold + warm64 ≈ 2.8×(vs fp32-fresh 표준구현) / 2.2×(vs bf16-fresh 재앵커), 정확도 전 축 fresh 동률.** 잔여: fp8 dequant-matvec Triton 커널로 speed 실측 앵커(트래픽 모델은 fp32 커브에서 검증됨).
- **★★★ Eval 커버리지 완결 (2026-07-09)**: ① commonsense **8/8 lossless**(v4=fresh, boolq/obqa 포함; SIQA만 datasets-deprecation 각주). ② **recall-intensive 6/6 lossless** — naive 엔진 생성 degenerate 함정 발견 후 **native decode에 v4 구현**(`v4_native_decode.py`, pb128 게이트 통과): fresh vs v4 평균 41.3/41.0(−0.3 노이즈), retrofit 자체도 생성 무손실(=raw). 실배포 의미론(prefill=warmup). 구현 함정 4건은 메모리 참조(특히: **native decode는 act 2D 호출이라 ActRotMask 스킵** — 회전 ckpt는 반드시 디스패처 경유). ③ 남은 갭: RULER/full-set/MMLU/GSM8K(camera-ready 급). 코드: `run_recall_native.py`, `gate_v4_native.py`, json `nemo9b_recall_nat_{fresh,v4}.json`.
...
## 6. 파일 지도
- `EVAL_LEDGER.md` — **★ 체크포인트 단계별 3-arm(raw/fresh/v4-c4) 정확도 원장 (전 태스크별 값):** 유지규칙(항상 3-arm, 방법론 바뀔 때마다 갱신) + p4long 전 스택 완비(commonsense/recall/RULER11/GSM8K/HumanEval/minerva) + minerva만 −2.9 잔존(long-CoT 재학습 처방) + longcot ckpt 예약 슬롯.
- `TRAIN_REPRO.md` — **★ 9B retrofit 처음부터 재현 레시피 (단일 소스):** 환경·데이터prep·ckpt 4단계 resume 체인(raw→qr→v4aware→p4mixed→p4long) 정확한 커맨드+계보표+평가재현+함정. `.pt`가 git 미포함이라 유일 재현 경로.
