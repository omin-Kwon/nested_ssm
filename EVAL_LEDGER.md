...
## E-T1: M 명시 추정 vs 학습 R (2026-07-14, probe_M_spectrum.py, wt103 12k tok)
평균(27층): top32 질량 0.473(균등 0.25) / **gap@32 ≈ 0.0001** / **학습R-vs-고유공간 cos 0.438 ≈ 무작위(0.5)** / 정렬도 0.398(무작위 0.33).
**판정: "SGD=명시 E[CBᵀ]의 고유분해" 기각** — 학습 R은 크기-통계의 고유기저와 사실상 무상관. 그러나 acc(항등 33.0 vs 학습 81.6)는 R의 정렬을 증명 → **중요도 = loss-민감도 가중 "실효 M"이며 크기 통계로 대체 불가** — 에너지≠recall 함정·ktop 붕괴·GHOST −26.5의 공통 뿌리. 논문 주장: 명시 통계는 학습 기저를 예측 못함 → calibration류 실패, end-to-end 학습 필수.
후속 E-T1b: lag-가중 교차공분산 M(τ) = E[C_t B_{t−τ}ᵀ]·decay^τ 로 재측정 (lag-0 근사 캐비앗 해소). json results/M_spectrum.json.
